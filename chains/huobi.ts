...
export default class HuobiLogic extends CoinInterface {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (transaction.rawTransaction) {
                this.web3.eth.sendSignedTransaction(transaction.rawTransaction, (err, id) => {
                    ...
                    const url = this.network === 'mainnet' ? `	https://hecoinfo.com/tx/${id}` : `https://testnet.hecoinfo.com/tx/${id}`;
    ...
    async getCurrentGasPrices() {
        let response = await axios.get('https://tc.hecochain.com/price/prediction');
        let prices = {
            low: response.data.prices.low,
            medium: response.data.prices.median,
            high: response.data.prices.fast
        };
        return prices;
    }
    /**
