...
/**
 * Fantom Logic config interface
 */
interface FantomLogicConfig {
    network: string | undefined,

};
/**
 * FantomLogic to handle the balance request and funds transfer to Fantom network
 * @class FantomLogic
 * {@link CoinInterface} 
 * 
 */
export default class FantomLogic extends CoinInterface {
    private network: string = DEFAULT_NETWORK;
    private fantomNetwork: string = 'mainnet';
    private chainId: number = 250;
    private chain: string = DEFAULT_NETWORK;
    private web3 = new Web3(new Web3.providers.HttpProvider(this.fantomNetwork));
    /**
     * Constructor of FantomLogic class
     * @param config network configuration
     */
    constructor(config: FantomLogicConfig) {
        super();
        this.setConfig(config);
    }
    /**
     * Set configuration for the Fantom logic class
     * @param config 
     */
    setConfig(config: FantomLogicConfig) {
        this.network = config && config.network ? config.network : DEFAULT_NETWORK;
        
        this.fantomNetwork = MAKE_NETOWRK_URL(this.network); // config.fantomNetwork; 
        this.chainId    = CHAIN_IDS(this.network); // config.ethChainId; // 4
        this.chain      = this.network; // config.ethChain; 
        this.web3 = new Web3(new Web3.providers.HttpProvider(this.fantomNetwork));
    }
    /**
    ...
    /**
     * Transfer funds from one Fantom address to others
     * 
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (transaction.rawTransaction) {
                this.web3.eth.sendSignedTransaction(transaction.rawTransaction, (err, id) => {
                    ...
                    const url = this.network === 'mainnet' ? `	https://ftmscan.com/tx/${id}` : `https://testnet.ftmscan.com/tx/${id}`;
